...
highscores(Context) ->
    z_db:assoc("SELECT disco_player.name, connected, SUM(score) AS score FROM disco_log JOIN disco_player ON (disco_log.player_id = disco_player.id) WHERE disco_log.score IS NOT NULL GROUP BY disco_player.name, connected ORDER BY SUM(score) DESC", Context).
