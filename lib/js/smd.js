...
app.factory('browserSupported', function() {
    return function() {
        ...
        if (ua.match(/Android/i) && !ua.match(/Chrome/i)) return false; // On Android, only support Chrome.
...
app.controller('mainCtrl', function($scope, $rootScope, $location, $timeout, appSocket, player, idlePlayer) {
    ...
    player.addEventListener('timeupdate', function() {
        $scope.$apply(function() {
            ...
            if (player.currentTime >= player.duration*0.99 && $scope.status == 'playing_final_song') {
                $location.path('/stopped');
            }
