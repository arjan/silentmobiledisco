...
app.controller('registerCtrl', function($scope, $location, $rootScope, appSocket) {
    ...
    $scope.start = function() {
        ...
        socket.cast('disco_register', {name: $scope.name});
        $location.path('/main');
    };
});
    ...
    $rootScope.$on('$viewContentLoaded', function() {
        ...
        player.addEventListener('progress', function() {
            if (player.buffered.length > 0 && player.buffered.end && player.duration) {
                percent = player.buffered.end(0) / player.duration;
            } else if (player.bytesTotal !== undefined && player.bytesTotal > 0 && player.bufferedBytes !== undefined) {
                percent = player.bufferedBytes / player.bytesTotal;
            }
            ...
            $scope.$apply(function() {
                $scope.loadPercent = percent;
                $scope.bufferedBytes = player.bufferedBytes;
                $scope.bytesTotal = player.bytesTotal;
            });
